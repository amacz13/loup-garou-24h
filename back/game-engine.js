...
const players = [
    ...
    .map(({ value }) => value);
export async function doDayVote(playerList) {
    ...
    voteMap.forEach((value, key) => {
        ...
        if (invertedVoteMap.has(value))
            invertedVoteMap.set(value, [...invertedVoteMap.get(value), key]);
        else
            invertedVoteMap.set(value, [key]);
        if (value > maxVote)
            maxVote = value;
    });
